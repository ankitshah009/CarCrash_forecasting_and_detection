"""
A script to train Faster R-CNN with CADP dataset.
"""
import keras.backend as K
from keras.layers import Input
from keras.models import Model
from keras.optimizers import Adam
from analysis.keras_frcnn import config, data_generators
from analysis.keras_frcnn import losses as losses_fn
from analysis.keras_frcnn import roi_helpers
from keras.utils import generic_utils
from analysis.keras_frcnn import resnet as nn
from improved_frcnn import resnet as nn_context
from analysis.keras_frcnn.simple_parser import get_data

import os, pickle, pprint, random, time
import numpy as np

ANNOTATION_CSV="./cadp_train.csv"
from analysis.logger import create_logger

LOGGER = create_logger("./data/train_frcnn_context.log", "Faster R-CNN Context")


def train_cadp(annotation_csv):
    cfg = config.Config()
    cfg.use_horizontal_flips = True
    cfg.use_horizontal_flips = True
    cfg.num_rois = 32
    cfg.base_net_weights = os.path.join('/home/tuananhn/.keras/models/', nn.get_weight_path())
    cfg.model_path = 'model_trained/model_frcnn_context_2_4.vgg.hdf5'
    # START contextual parameters
    cfg.n_contexts = 2
    cfg.step_stride = 4
    # END contextual parameters
    all_images, classes_count, class_mapping = get_data(annotation_csv)
    LOGGER.info(msg=str(cfg))
    ...
    LOGGER.info('Training images per class: {}'.format(classes_count))
    ...
    LOGGER.info('Num train samples {}'.format(len(train_imgs)))
    ...
    classifier = nn_context.classifier(cfg.step_stride, cfg.n_contexts,
                                       shared_layers, roi_input, cfg.num_rois,
    ...
    try:
        LOGGER.info('loading weights from {}'.format(cfg.base_net_weights))
        ...
        LOGGER.error(e)
        LOGGER.error('Could not load pretrained model weights. Weights can be found in the keras application folder '
    ...
    for epoch_num in range(num_epochs):
        ...
        LOGGER.info('Epoch {}/{}'.format(epoch_num + 1, num_epochs))
            ...
            try:
                ...
                if len(rpn_accuracy_rpn_monitor) == epoch_length and cfg.verbose:
                    ...
                    if mean_overlapping_bboxes == 0:
                        LOGGER.info('RPN is not producing bounding boxes that overlap'
